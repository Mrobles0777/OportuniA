...
import { X, Mail, Lock, User, Phone, DollarSign, Loader2, ArrowRight, LogIn, AlertCircle, Clock, Inbox, Send, ShieldAlert, Zap } from 'lucide-react';
...
const AuthModal: React.FC<Props> = ({ onClose, onSuccess, initialMode = 'register' }) => {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-end md:items-center justify-center p-0 md:p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-300">
      <div className="bg-white rounded-t-[2.5rem] md:rounded-[2.5rem] w-full max-w-md overflow-hidden shadow-2xl animate-in slide-in-from-bottom md:zoom-in duration-300">
        <div className="p-8">
          <div className="flex justify-between items-center mb-6">
            <div>
              <h2 className="text-2xl font-black text-slate-900 uppercase tracking-tight">
                {mode === 'register' ? 'Unirse a OportuniA' : 'Hola de nuevo'}
              </h2>
              <p className="text-sm text-slate-500 font-medium">
                {mode === 'register' ? 'Acceso a nichos de mercado con IA.' : 'Ingresa a tu panel de control.'}
              </p>
            </div>
          ...
          <div className="space-y-3 mb-6">
            <button
              ...
              className="w-full py-4 bg-white border border-slate-200 text-slate-700 rounded-2xl font-black text-xs uppercase tracking-widest flex items-center justify-center gap-3 hover:bg-slate-50 transition-all shadow-sm active:scale-[0.98] disabled:opacity-50"
            >
              <svg className="w-5 h-5" viewBox="0 0 24 24">
                <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" />
                <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" />
                <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l3.66-2.84z" />
                <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 6.57l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" />
              </svg>
              Google
            </button>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button
              disabled={loading || cooldown > 0}
              type="submit"
              className={`w-full py-4 rounded-2xl font-black text-xs uppercase tracking-widest flex items-center justify-center gap-2 transition-all shadow-lg mt-4 ${cooldown > 0
                ? 'bg-slate-300 text-slate-500 cursor-not-allowed border border-slate-400'
                : 'bg-indigo-600 hover:bg-indigo-500 text-white active:scale-95 shadow-indigo-200'
                }`}
            >
              {loading ? (
                <>
                ...
                <>
                  {mode === 'register' ? 'Comenzar Ahora' : 'Acceder'}
                  {mode === 'register' ? <ArrowRight className="w-4 h-4" /> : <Zap className="w-4 h-4" />}
                </>
              )}
            </button>
          </form>
